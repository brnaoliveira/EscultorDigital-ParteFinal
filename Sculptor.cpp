...
/*
 * função para averiguar se as coordenadas passadas como parametro pertencem ao espaço tridimencional com tamanho informado
 * (x,y,z) são as coordenadas que seram verificadas
 * (nc,na,nl) são as dimensões do espaço 3D que será usado, correspondentes a (nx,ny,nz) respectivamente
*/
bool verificaCord(int x, int y, int z,int nc, int na, int nl)
{
    if(x>=0 && x<=nc){
        if(y>=0 && y<= na){
            if(z>=0 && z<=nl){
...
void Sculptor :: putBox(int x0, int x1, int y0, int y1, int z0, int z1)
{
    ...
    if(x1>=x0&&y1>=y0&&z1>=z0){
        for(i=z0;i<=z1;i++){
            for(j=x0;j<=x1;j++){
                for(w=y0;w<=y1;w++){
                    if(verificaCord(j,w,i,nx,ny,nz)){
                        putVoxel(j,w,i);
                    }
                }
            }
...
{
    if(verificaCord(xcenter,ycenter,zcenter,nx,ny,nz)){
        int i,j,w;
        for(i=0;i<nz;i++)
        {
            for(j=0;j<nx;j++)
